...
#
# Description: config to simulate M0 biodiversity experiment starting from South America.
#
######################################
...
create_ancestor_species <- function(landscape, config) {
  # Coordinates of South American grid cells included in our landscape
  South_America_coordinates <- landscape$coordinates[which(landscape$coordinates[,1] < -33 & 
                                                             landscape$coordinates[,2] > 6),] 
  # Initial environment
  South_America_environment <- landscape$environment[which(landscape$coordinates[,1] < -33 & 
                                                             landscape$coordinates[,2] > 6), 1]
  # Rasterise
  South_America_raster <- raster::rasterFromXYZ(cbind(South_America_coordinates, South_America_environment))
  # Extend initial raster to reach the extent of the landscape
  South_America_raster <- extend(South_America_raster, landscape$extent)
  
  #Set up ancestral species
  South_America_start_cells  <- sample(Which(South_America_raster, cells=T), 1)
  South_America_start_cells  <- as.character(South_America_start_cells)
  
  # now fill the list of new species
  South_America_species <- create_species(South_America_start_cells, config)
  South_America_species$traits[ , "temp_optimum"] <- median(landscape$environment[South_America_start_cells, "temp"], na.rm=T)
  South_America_species$traits[ , "prec_optimum"] <- median(landscape$environment[South_America_start_cells, "prec"], na.rm=T)
  
  all_species <- list(South_America_species)
...
grid_cell_distance <- params$grid_cell_distance # raster resolution
