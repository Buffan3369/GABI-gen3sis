...
## QUANTIFICATIONS AND STATISTICAL ANALYSIS {#sec-quanti}
...
Finally, to test whether simulated exchange pulses could relate to Plio-Pleistocene climate fluctuations--as postulated in the literature--for each successful simulation, we quantified the timings of all trans-continental dispersal events. This timing assessment was achieved through a one-by-one scanning of the presence/absence matrices of all simulated species from their time of origination onwards, assessing the time at which their presence on the landmass to colonise was first recorded -- provided it colonised that landmass. Then, for each ancestral continent-model combination, we matched the obtained distribution of exchange timings to the temperature curve of the ancestral continent (@fig-exchange_timing). We only focused on simulated species that originated on the ancestral continent, *i*.*e*., not resulting from the radiation of migrant lineages on the colonised continent.
...
#| out-height: 12cm
#| out-width: 17cm
#| fig-cap: Distribution of the timing of northwards (*left*) and southwards (*right*) dispersal events across successful simulations, either generated by the $M0$ (*bottom*) or $M1$ (*bottom*) models. Histograms illustrate the frequency of exchanges retrieved throughout simulatins per 100ky-long time bin (see @sec-quanti). Brown curves in the background show continent-averaged annual temperature (BIO1) for either North or South America, coming from the PALEO-PGEM emulator @holden2019. Geological timescale was displayed thanks to the deeptime R package @gearty2023.
...
#| out-height: 17cm
#| out-width: 23cm
#| fig-cap: Proportion of the final area occupied within each biome after each successful simulation.
