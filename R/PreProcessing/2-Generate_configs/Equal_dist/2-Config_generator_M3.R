################################################################################
# Goal: Generate configs for the M3 scenario starting from North America after
...
if(!dir.exists("./Data/Gen3sis_parameter_tables/Equal_dist/M3")){dir.create("./Data/Gen3sis_parameter_tables/Equal_dist/M3")}
write.tbl.std(param_df, 
              paste0("./Data/Gen3sis_parameter_tables/Equal_dist/M3/", start_region, "_parameters_EQ_Dist.txt"))

## Access the distances to isthmus chosen for South American species -----------
param_tbl_SA_M3 <- read.table("./Data/Gen3sis_parameter_tables/M3/South_America_parameters_EXTENDED_EXCH_AREA_DIV_DIST_biome.txt",
                              header = TRUE)
init_dists <- param_tbl_SA_M3$dist_to_isthmus
...
if(length(unique(is.na(init_dists))) > 1){
  ...
  for(idx in is_na_dist){
    init_dists[idx] <- rnorm(n = 1, 
                             mean = mean(param_tbl_SA_M3$dist_to_isthmus, na.rm = T), 
                             sd = sd(param_tbl_SA_M3$dist_to_isthmus, na.rm = T))
  }
}
param_df$dist_to_isthm <- init_dists

## Generate config files -------------------------------------------------------
out <- paste0("./Data/Gen3sis_configs/Equal_distance/M3/", start_region, "_start")
for(i in 1:n){
  ...
  writeLines(config_i, paste0(out_dir, "/config_M3_", start_region, "_run_", i, ".R"))
}  
